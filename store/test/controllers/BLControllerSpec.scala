import org.specs2.mutable.Specification
import play.api.inject.guice.GuiceApplicationBuilder
import play.api.test.CSRFTokenHelper._
import play.api.test.Helpers._
import play.api.test.{ FakeRequest, WithApplication }
import com.dsleng.controllers.{ApplicationController,BLController}
import play.mvc.Http.HeaderNames
import play.api.libs.json.Json

class BLControllerSpec extends Specification {
//  "ApplicationController POST" should {
//
//    "render the index page from the application" in new WithApplication() {
//
//      //val controller = app.injector.instanceOf[ApplicationController]
//      val controller = app.injector.instanceOf[BLController]
//
//      val requestNode = Json.toJson(Map("name" -> "Testname"))
//    //   val request = FakeRequest().copy(body = requestNode)
//    //     .withHeaders(HeaderNames.CONTENT_TYPE -> "application/json");
//
//      //val request = FakeRequest().withCSRFToken
//      val request = FakeRequest(POST, "/api/query")
//        .withJsonBody(Json.parse("""{ "field": "value" }"""))
//        .withHeaders("Authorization" -> "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..O0X4-6eFlYnnZTyX2VL6VZ741XtvbvY9xTMFsLEfATw")
//        .withCSRFToken
//      val result = controller.query().apply(request)
//      println("results")
//      println(result)
//      status(result) must beEqualTo(OK)
//      contentType(result) must beSome("application/json")
//      println(contentAsString(result))
//    }
//  }
    "BLController POST" should {

    "render the index page from the application" in new WithApplication() {

      val controller = app.injector.instanceOf[BLController]

      val requestNode = Json.toJson(Map("name" -> "Testname"))
    //   val request = FakeRequest().copy(body = requestNode)
    //     .withHeaders(HeaderNames.CONTENT_TYPE -> "application/json");

      //val request = FakeRequest().withCSRFToken
      val request = FakeRequest(POST, "/api/genentropy")
        .withJsonBody(Json.parse("""{ "field": "value" }"""))
        .withHeaders("Authorization" -> "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..O0X4-6eFlYnnZTyX2VL6VZ741XtvbvY9xTMFsLEfATw")
        .withCSRFToken
      val result = controller.genEntropy().apply(request)
      println("results----------------------------->")
      println(result)
      status(result) must beEqualTo(OK)
      contentType(result) must beSome("application/json")
      println(contentAsString(result))
    }
  }
}